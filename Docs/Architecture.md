...
*   **Volume identity**: `VolumeId` is what two providers compare to decide they share a volume. It gates the atomic-move fast path and keys the per-volume caches (drive classification, free space), so it must be the *volume*, never the root path — the drive root on Windows, the deepest containing mount point on Unix (`MountPointResolver`; .NET's `DriveInfo(string)` does not resolve mount points on Unix), and null for network paths. Treat it as best-effort: consumers must degrade gracefully both when it is null (no same-volume claim) and when a claim proves over-optimistic (macOS firmlinks).
