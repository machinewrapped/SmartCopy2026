...
- **Gate 3: non-Windows platform validation.** The limited macOS validation pass found no correctness issue or run-level regression, so the same optimised default is now used cross-platform. It ran single-clone with no path-pool rotation, so its Tiny/Small bucket evidence is softer than the Windows equivalent ([Production Validation Pass](optimisation-strategies-production-validation.md), post-validation section). Linux remains an explicit cross-platform assumption pending native validation.
